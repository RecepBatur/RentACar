...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminPricingController : Controller
    {
        ...
        public async Task<IActionResult> CreatePricing(CreatePricingDto createPricingDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/Pricings", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/Pricings?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminPricing", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdatePricing(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdatePricingDto>(jesonData);
                return View(values);
            }
            return View();
        }
