...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminAuthorController : Controller
    {
        ...
        public async Task<IActionResult> CreateAuthor(CreateAuthorDto createAuthorDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/Authors", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/Authors?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminAuthor", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateAuthor(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdateAuthorDto>(jesonData);
                return View(values);
            }
            return View();
        }
