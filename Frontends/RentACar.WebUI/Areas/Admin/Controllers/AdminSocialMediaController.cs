...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminSocialMediaController : Controller
    {
        ...
        public async Task<IActionResult> CreateSocialMedia(CreateSocialMediaDto createSocialMediaDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/SocialMedias", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/SocialMedias?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminSocialMedia", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateSocialMedia(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdateSocialMediaDto>(jesonData);
                return View(values);
            }
            return View();
        }
