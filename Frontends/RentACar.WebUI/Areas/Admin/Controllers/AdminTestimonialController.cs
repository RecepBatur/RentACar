...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminTestimonialController : Controller
    {
        ...
        public async Task<IActionResult> CreateTestimonial(CreateTestimonialDto createTestimonialDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/Testimonials", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/Testimonials?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminTestimonial", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateTestimonial(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdateTestimonialDto>(jesonData);
                return View(values);
            }
            return View();
        }
