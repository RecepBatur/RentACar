...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminServiceController : Controller
    {
        ...
        public async Task<IActionResult> CreateService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/Services", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/Services?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminService", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7286/api/Services/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jesonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<UpdateServiceDto>(jesonData);
                return View(values);
            }
            return View();
        }
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto updateServiceDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7286/api/Services/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminService", new { area = "Admin" });
            }
