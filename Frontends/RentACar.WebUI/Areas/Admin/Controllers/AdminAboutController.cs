...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminAboutController : Controller
    {
        ...
        public async Task<IActionResult> CreateAbout(CreateAboutDto createAboutDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/Abouts", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/Abouts?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminAbout", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7286/api/Abouts/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jesonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<UpdateAboutDto>(jesonData);
                return View(values);
            }
            return View();
        }
