...
namespace RentACar.WebUI.Controllers
{
    public class AdminBrandController : Controller
    {
        ...
        public async Task<IActionResult> CreateBrand(CreateBrandDto createBrandDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7286/api/Brands", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7286/api/Brands?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateBrand(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdateBrandDto>(jesonData);
                return View(values);
            }
            return View();
        }
